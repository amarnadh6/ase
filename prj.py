...
def one_stn_all_day_avg(stn_no):
	...
	results_dir_1 = os.path.join(script_dir_1, 'one_stn_all_day_avg/')
	name_1 = title_1 + ' all days'
...
def all_stn_one_day_avg():
	...
	plt.legend()
	plt.title('Prices for all the stations for ' + title_2)
	plt.xlabel('Address')
	plt.ylabel('Price in cents')
	plt.xticks(x2, rotation=90)
	plt.gcf().subplots_adjust(bottom=.45)
	script_dir_2 = os.path.dirname(__file__)
	results_dir_2 = os.path.join(script_dir_2, 'all_stn_one_day_avg/')
	name_2 = 'One day'
	plt.savefig(results_dir_2 + name_2)
...
def one_stn_n_day_split(stn_no, no_of_days):
	...
	results_dir_3 = os.path.join(script_dir_3, 'one_stn_n_day_split/')
	name_3 = title_3 + ' last 5 days'
...
def one_stn_last_day_split(stn_no):
	...
	results_dir_4 = os.path.join(script_dir_4, 'one_stn_last_day_split/')
	name_4 = title_4 + ' last day'
...
def one_company_average(company):
	...
	#from this data, display address vs average
		...
		plt.savefig(results_dir_5 + company)
...
#one_stn_n_day_split(28,5)
#one_stn_all_day_avg(28)

one_company_average('Costco')
